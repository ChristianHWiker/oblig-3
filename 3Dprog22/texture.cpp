...
glGenTextures(1, &textureID); // here when texture is generated an ID is
//assigned to it
glBindTexture(GL_TEXTURE_2D, textureID); // we bind the texture here
glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT); //S Axis // we
//want to repeat the texture to fill the mesh
glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT); // T Axis
glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,GL_LINEAR); // we apply
//filters for min and mag as a linear
